...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        ...
        self.conv3_for_upsample3    = C2f(base_channels * 4 + base_channels * 2, base_channels * 2, base_depth, shortcut=False)
        self.down_sample0 = Conv(base_channels * 2, base_channels * 2, 3, 2)
        self.conv3_for_downsample0 = C2f(base_channels * 4 + base_channels * 2, base_channels * 4, base_depth,shortcut=False)
        ...
        ch              = [base_channels * 2, base_channels * 4, base_channels * 8, int(base_channels * 16 * deep_mul)]
        self.shape      = None
        self.nl         = len(ch)
    ...
    def forward(self, x):
        ...
        #
        # 256, 80, 80 => 256, 160, 160
        P3_upsample = self.upsample(P3)
        # 256, 160, 160 cat 128, 160, 160 => 384, 160, 160
        P2          = torch.cat([P3_upsample, feat0], 1)
        #384, 160, 160 = > 128, 160, 160
        P2          = self.conv3_for_upsample3(P2)
        #128, 160, 160 = > 128, 80, 80
        P2_downsample = self.down_sample0(P2)
        # 256, 80, 80 cat 128, 80, 80 => 384, 80, 80
        P1 = torch.cat([P2_downsample, P3], 1)
        # 384, 80, 80 => 128, 80, 80
        P1 = self.conv3_for_downsample0(P1)
        #
        # 256, 80, 80 => 256, 40, 40
        P1_downsample = self.down_sample1(P1)
        # 512, 40, 40 cat 256, 40, 40 => 768, 40, 40
        P4 = torch.cat([P1_downsample, P4], 1)
        # 768, 40, 40 => 512, 40, 40
