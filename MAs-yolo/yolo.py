...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'model_data/MAs-YOLO_X1.pth',
        "classes_path"      : 'model_data/ma_classes.txt',
        #---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
        #---------------------------------------------------------------------#
        "input_shape"       : [2304, 2304],
        #------------------------------------------------------#
        #   所使用到的yolov8的版本：
        #   n : 对应yolov8_n
        #   s : 对应yolov8_s
    ...
    def calculate_circle_area(self, left, right, top, bottom):
        # 计算矩形框的宽度和高度
        width = right - left
        height = bottom - top
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            bottom  = min(image.size[1], np.floor(bottom).astype('int32'))
            right   = min(image.size[0], np.floor(right).astype('int32'))
            area = self.calculate_circle_area(left, right, top, bottom)

            label = '{:.2f}'.format(area)
            draw = ImageDraw.Draw(image)
            label_size = draw.textsize(label, font)
            label = label.encode('utf-8')
            print(label, top, left, bottom, right, area)
            ...
            for i in range(thickness):
                draw.rectangle([left + i, top + i, right - i, bottom - i], outline='red', width=1)
